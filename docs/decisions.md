...
## ADR-009 이슈/todolist 관리 도구

- **날짜**: 2026-08-01
- **상태**: 확정
- **배경**: GitHub Issues만으로는 우선순위 정렬/보드 뷰가 부족해 별도 todolist 도구 필요성 검토.
  Linear는 2026-02 가격 개편 이후 GitHub 연동이 무료 플랜에서 제외되고 Business 플랜($19/user/월)부터
  필요해져 비용 조건에 맞지 않음.
- **결정**: Plane Community Edition(AGPL-3.0, 오픈소스) 셀프호스팅 도입.
  GitHub Issues와 라벨 기반 양방향 동기화(제목/설명/상태/담당자/라벨/댓글) 사용.
- **이유**:
  - 무료 (셀프호스팅, 인원 제한 없음) — 8/3 복직 이후 사이드 프로젝트 예산·시간이 줄어드는 상황에서 비용 최우선
  - GitHub Issues와 데이터를 이중 관리하지 않아도 됨 (동기화로 source of truth 분산 최소화)
  - 칸반/사이클 뷰로 우선순위·진행상태 시각화 가능 (Issues 단독의 갭 보완)
  - Infisical과 동일하게 셀프호스팅 인프라 운영 경험 축적 가능
- **트레이드오프 인지**: 직접 호스팅·운영 부담(백업, 업데이트) 발생. Infisical과 마찬가지로 맥북 내
  별도 경로에서 운영 예정, 세부 설치 방식은 별도 ADR로 후속 기록.
- **참고**: 설치/운영 세부사항(경로, Docker Compose 구성, GitHub 연동 설정)은 실제 구축 후
  ADR-010으로 추가 예정.